<template>
  <div v-if="mostrar" class="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-10"
    @keydown.escape="closeModal">
    <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full">
      <div class="px-4 py-5 sm:px-6">
        <div class="w-full flex flex-row justify-between items-center">
          <titleH2 class="mb-0" :class="`text-${theme}-text`"> {{ modo === 0 ? 'Nuevo' : 'Editar' }} tipo de comprobante
          </titleH2>
        ...
        <div class="flex flex-col justify-center items-start mt-8">
          ...
          <input :disabled="modo === 1" name="Clave" v-model="datos.ClaveTipoDeComprobante" type="text" placeholder="T"
        ...
        <div class="flex flex-col justify-center items-start mt-6">
          ...
          <input v-model="datos.Descripcion" type="text" placeholder="Traslado"
            class="mt-1 block w-full text-sm border-2 border-gray-400 transition-all duration-300 focus:border-gray-800 box-border p-2 outline-none">
        </div>
