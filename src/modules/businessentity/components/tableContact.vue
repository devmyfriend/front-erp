...
const test1 = () => {
    console.log('test1');
    console.log('ListaContacto: \n' + JSON.stringify(props.ListaContacto));
    ...
    <table class="w-full table-fixed leading-4 text-[1rem]" @click="test1">
        <thead>
            <tr class="sticky top-0 h-primaryHeaderTableHeight text-white">
                ...
                <th :class="`bg-${theme}-headers`">Apellido Paterno</th>
                <th :class="`bg-${theme}-headers`">Apellido Materno</th>
                <th v-if="editable" :class="`bg-${theme}-headers`"> Acciones </th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="(email, index) in ListaContacto" :key="index"
